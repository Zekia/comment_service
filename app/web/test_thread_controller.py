...
def test_put_thread_should_be_idempotent():
    thread_id = str(uuid.uuid4())
    httpx.put(f"http://localhost/threads/{thread_id}", json={'title': 'my_title'})
    response = httpx.put(f"http://localhost/threads/{thread_id}", json={'title': 'my_title'})
    # Assert HTTP code and if received document id is valid
...
def test_get_one_thread_should_return_thread():
    ...
    thread_id = str(uuid.uuid4())
    response = httpx.put(f"http://localhost/threads/{thread_id}", json={'title': expected_title})
...
def test_update_one_thread():
    old_title = 'my_title'
    thread_id = str(uuid.uuid4())
    response = httpx.put(f"http://localhost/threads/{thread_id}", json={'title': old_title})
    thread_id = response.json()['_id']

    new_title = 'new_title'
    responseUpdate = httpx.put(f"http://localhost/threads/{thread_id}", json={'title': new_title})

    assert responseUpdate.status_code == 200
    responseFind = httpx.get(f"http://localhost/threads/{thread_id}")
    assert new_title == responseFind.json()['title']

def test_remove_one_thread():
    thread_id = str(uuid.uuid4())
    response = httpx.put(f"http://localhost/threads/{thread_id}", json={'title': 'title'})
    thread_id = response.json()['_id']

    response = httpx.delete(f"http://localhost/threads/{thread_id}")
    assert response.status_code == 200
    assert response.json()['id'] == thread_id
    find_response = httpx.get(f"http://localhost/threads/{thread_id}")
